...
const Searchbar = (props) => {
    ...
    const handleSubmit = (event) => {
        ...
        searchComics(search.query, search.parameter, false)
    }
    const searchComics = (query, type, exactMatch) => {
        props.setComics([])
        fetch(`${url}/database/search?query=${query}&type=${type}&exactMatch=${exactMatch}`)
            .then((response) =>
                response.json())
            .then((data) => {
            ...
            <ButtonGroup className={'w-100 d-flex flex-row justify-content-center align-items-center'}>
                    ...
                    <Dropdown
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownItem onClick={() => handleItemClick("runs")}>Runs</DropdownItem>
                            <DropdownItem onClick={() => handleItemClick("gaps")}>Gaps</DropdownItem>
                            <DropdownItem onClick={() => handleItemClick("grade")}>Graded</DropdownItem>
                            <DropdownItem onClick={() => handleItemClick("slab")}>Slabbed</DropdownItem>
                            <DropdownItem onClick={() => handleItemClick("sign")}>Signed</DropdownItem>
                        </DropdownMenu>
                    </Dropdown>
            </ButtonGroup>
            </form>
